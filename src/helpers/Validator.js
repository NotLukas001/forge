...
const { BOT_TOKEN, MONGO_CONNECTION, SPOTIFY_CLIENT_ID, SPOTIFY_CLIENT_SECRET, BOT_SECRET } = require("@root/config");
...
module.exports = class Validator {
  static validateConfiguration() {
    ...
    if (config.DASHBOARD.enabled) {
      if (!BOT_SECRET) {
        error("BOT_SECRET cannot be empty");
        process.exit(1);
      }
    ...
    if (config.MUSIC.ENABLED) {
      if (!SPOTIFY_CLIENT_ID || !SPOTIFY_CLIENT_SECRET) {
        warn("SPOTIFY_CLIENT_ID or SPOTIFY_CLIENT_SECRET are missing. Spotify music links won't work");
      }
    ...
    if (config.WEATHERSTACK_KEY) warn("WEATHERSTACK_KEY is missing. Weather command won't work");
    if (config.STRANGE_API_KEY) warn("STRANGE_API_KEY is missing. Image commands won't work");
  }
