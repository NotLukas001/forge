...
const { EMBED_COLORS, SUPPORT_SERVER, DASHBOARD, WEBSITE, imageHELPMENU } = require("@root/config.js");
const {
  EmbedBuilder,
  ActionRowBuilder,
...
async function getHelpMenu({ client, guild }) {
  ...
  let components = [];
  components.push(
    new ButtonBuilder().setCustomId("previousBtn").setEmoji("<:kiri:1168926892615815210>").setStyle(ButtonStyle.Secondary).setDisabled(true),
    new ButtonBuilder().setCustomId("nextBtn").setEmoji("<:kanan:1168926869786218526>").setStyle(ButtonStyle.Secondary).setDisabled(true)
  );
  ...
  .setAuthor({
    name: `${guild.members.me.displayName} Help Command!`,
    iconURL: client.user.displayAvatarURL(),
  })  
  .setColor(EMBED_COLORS.BOT_EMBED)
  .setTitle("This is a multi-purpose bot that provides moderation and music features<a:acaaah:1167688811560579123>")
  .setDescription(
      `<:blank:1173965100311379978>**Support Server:** [**Join**](${SUPPORT_SERVER})\n` +
      `<:blank:1173965100311379978>**Website:** [**Visit**](${WEBSITE})\n` +
      `<:blank:1173965100311379978>**Dashboard** [**Visit**](${DASHBOARD.baseURL})`
  )
  .setImage(imageHELPMENU)
...
const waiter = (msg, userId, prefix) => {
  ...
  collector.on("collect", async (response) => {
    ...
    clearTimeout(resetTimeout); // Reset the timeout
  ...
  const resetToInitialState = async () => {
    ...
    // Hapus opsi menu dari menuRow
    menuRow.components = [];
    
    // Hapus tombol previousBtn dan nextBtn
    buttonsRow.components = buttonsRow.components.filter(button => button.customId !== "previousBtn" && button.customId !== "nextBtn");
  ...
  collector.on("end", () => {
    ...
    msg.editable && msg.edit({ components: [] });

    // Set a new timeout to reset the menu to the beginning after timeout
    resetTimeout = setTimeout(resetToInitialState);
  });
}
...
function getSlashCategoryEmbeds(client, category) {
  ...
  while (commands.length) {
    ...
    toAdd = toAdd.map((cmd) => {
      ...
      return `\`/${cmd.name}\`\n <a:tunesia_prefix:1167678316770238584> **Description**: ${cmd.description}\n ${
...
function getMsgCategoryEmbeds(client, category, prefix) {
  ...
  while (commands.length) {
    let toAdd = commands.splice(0, commands.length > CMDS_PER_PAGE ? CMDS_PER_PAGE : commands.length);
    toAdd = toAdd.map((cmd) => `\`${prefix}${cmd.name}\`\n <a:tunesia_prefix:1167678316770238584> ${cmd.description}\n`);
    arrSplitted.push(toAdd);
  }
  ...
  arrSplitted.forEach((item, index) => {
    const embed = new EmbedBuilder()
      ...
      .setDescription(item.join("\n"))
      .setFooter({
        text: `page ${index + 1} of ${arrSplitted.length} | Type ${prefix}<command> for more command information`,
      });
    arrEmbeds.push(embed);
  });
