...
const buildGreeting = async (member, type, config, inviterData) => {
  ...
  if (!config.content && !config.embed.description && !config.embed.footer) {
    const mention = member.toString(); // Menghasilkan mention pengguna
    const serverName = member.guild.name; // Nama server
    content =
      type === "WELCOME"
        ? `Welcome to ${serverName}, ${mention}! 🎉.`
        : `${mention} has left ${serverName} 👋.`;
